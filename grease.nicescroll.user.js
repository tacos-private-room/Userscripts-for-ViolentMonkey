...
(function(){
    ...
    function main() {
      ...
      var fbPlug = /\.facebook\.com\/plugins(\/|$)/.test(href);
      ...
      function loadNS() {    
        ...
        if (typeof j.fn.niceScroll == "undefined") {
          ...
          loadScript(
            'http://areaaperta.com/nicescroll/js/jquery.nicescroll.min.js',
            function() {
              var scroll = getScrollable();
              j('textarea').css('overflow-y','auto');
            
              if (isfacebook != fbPlug) {
                ...
                j('#fbNotificationsFlyout').this.niceScroll().hide();
                (j(".plugin")||j(".transparent_widget")).this.niceScroll().hide();
                ...
                nice = scroll.niceScroll({cursorcolor:'#0084B4',railoffset:{left:-2}});
              }
              else if (isgoogle) {
...
*/                  
              ...
              else if (j('body[data-twttr-rendered="true"]')) {
                nice = scroll.niceScroll();
              }
              else  {
                nice = scroll.niceScroll();
